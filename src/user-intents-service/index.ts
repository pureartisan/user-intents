...
export class UserIntentService {
  ...
  private eventHandlers: Map<EventType, EventHandler<EventListener>> = new Map<
    EventType,
  ...
  private warnNonExistingEvent(name: string, action: string): void {
    ...
    this.warn(
      `Trying to ${action} a non-existing intent: ${name}. Did you forget to start it or has already completed/failed?`
    );
  }

  private initEventHandlers(): void {
    const types: EventType[] = ['completed', 'incompleted', 'cancelled', 'failed', 'timedout'];
    types.forEach((type) => {
